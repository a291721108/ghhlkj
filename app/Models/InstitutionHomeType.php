...
class InstitutionHomeType extends Common
{
    ...
    {
        return self::where('institution_id', $homeTpyeId)->where('status', '>', self::Home_TYPE_SYS_STATUS_TWO)->select('id','home_type','home_price')->get()->toArray();
    }
    ...
    {
        return self::where('id', $id)->select('id','home_type','home_price','home_detail')->get()->toArray();
    }
